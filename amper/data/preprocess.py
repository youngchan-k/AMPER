"""Data preprocessing for EdNet-KT1."""
...
def plot_tag_distribution(question_csv):
    ...
    for i in range(len(question_info)):
        ...
        for j in range(len(question_tags)):
            all_tag.append(int(question_tags[j]))
    ...
    plt.savefig(os.path.join(config.PROJECT_ROOT, "question_tags.png"))
    ...
    plt.savefig(os.path.join(config.PROJECT_ROOT, "question_tags_prop.png"))


def save_preprocessd_data(file_dir, save_dir, question_csv):
    file_list = os.listdir(file_dir)
    file_list = random.sample(file_list, len(file_list) // 100)

    os.makedirs(save_dir, exist_ok=True)

    question_info = pd.read_csv(
        question_csv, usecols=["question_id", "correct_answer", "part", "tags"]
    )
    question_tags, all_tag, prop_tag = [], [], []
    ...
    question_info_del.to_csv(config.QUESTIONS_PROCESS_CSV, index=False)
    ...
    for file_name in file_list:
        ...
        question_data = question_info_re.loc[
            user_info.loc[0 : len(user_info)].question_id
        ]
        user_data = pd.concat(
            [user_info, question_data.reset_index(drop=True)], axis=1
        )
        user_data["user"] = [
            file_name.replace(".csv", "") for _ in range(len(user_data))
        ]
        user_data = user_data[
            [
                "user",
        ...
        for i in range(len(user_info)):
            if user_data.loc[i].question_id not in list(
                question_info_del.question_id
            ):
...
def save_merged_data(file_dir, save_dir, input_csv):
    ...
    for file_name in data_list:
        ...
        user_data = user_data.iloc[
            0 : len(user_data) - len(user_data) % 21
        ]
        if len(user_data) != 0:
            user_data.to_csv(os.path.join(save_dir, file_name), index=False)
...
def split_data(input_csv, train_csv, label_csv):
    ...
    label_df = pd.concat(label_list, axis=1).transpose()
...
def main_data_preprocess(
    ...
    file_dir = file_dir or config.DATA_DIR
    save_dir = save_dir or config.DATA_PREPROCESS_DIR
    save_dir_final = save_dir_final or config.DATA_PREPROCESS_FINAL_DIR
    question_csv = question_csv or config.QUESTION_CSV
    input_csv = input_csv or config.INPUT_CSV
